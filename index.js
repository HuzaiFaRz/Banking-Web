...
function registerFormLoginForm() {
  registerForm.addEventListener("submit", function (e) {
    ...
    if (registerUserFirstName.value === "") {
      registerErrorMsg = "Please Fill First Name Field";
    } else if (registerUserLastName.value === "") {
      registerErrorMsg = "Please Fill Last Name Field";
    } else if (registerUserEmail.value === "") {
      registerErrorMsg = "Please Fill Email Field";
    } else if (registerUserPassword.value === "") {
      registerErrorMsg = "Please Fill Password Field";
    } else if (registerUserPassword.value.length < 8) {
      registerErrorMsg = "Password Must Be At Least 8 Characters";
    } else {
      var userKey = "user_" + registerUserEmail.value;
      if (localStorage.getItem(userKey)) {
        registerErrorMsg = "User Already Exists. Please log in.";
      } else {
        var registerFormDataStore = {
          fName: registerUserFirstName.value,
          lName: registerUserLastName.value,
        ...
        localStorage.setItem(userKey, JSON.stringify(registerFormDataStore));
        registerErrorMsg = "Registered Successfully!";
        userAcountUserName.innerHTML =
          "Hi, " +
          registerFormDataStore.fName +
          " " +
    ...
    registerFormError.innerHTML = registerErrorMsg;
    registerFormError.style.color = registerErrorMsg.includes("Successful")
      ? "green"
      : "red";
    registerForm.reset();
  });
  ...
  loginForm.addEventListener("submit", function (j) {
    ...
    var loginErrorMsg = "";
    if (loginUserEmail.value === "") {
      loginErrorMsg = "Please Fill Email Field";
    } else if (loginUserPassword.value === "") {
      loginErrorMsg = "Please Fill Password Field";
    } else {
      var userKey = "user_" + loginUserEmail.value;
      var storedUserData = JSON.parse(localStorage.getItem(userKey));
      if (storedUserData && storedUserData.email === loginUserEmail.value) {
        ...
        loginErrorMsg = "User Not Found. Please Register.";
      }
    }
    loginFormError.innerHTML = loginErrorMsg;
    loginFormError.style.color = loginErrorMsg.includes("Successful")
      ? "green"
      : "red";
...
function deposit() {
  depositBtn.addEventListener("click", function (event) {
    ...
    if (isNaN(depositAmount) || depositAmount <= 0) {
      ...
      var currentBankBalance = parseFloat(localStorage.getItem("amount")) || 0;
